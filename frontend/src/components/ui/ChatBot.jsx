...
const ChatBot = () => {
    ...
    const [messages, setMessages] = useState([
        { text: "Hi! I am Ganesh's AI Assistant. Ask me anything about Web Dev!", sender: "bot" },
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages((prev) => [...prev, { text: "⚠️ Error connecting to server.", sender: "bot", isError: true }]);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="bg-gray-900 border border-gray-700 w-[90vw] sm:w-[350px] md:w-[380px] h-[500px] max-h-[80vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden mb-4">
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-4 flex justify-between items-center shadow-lg">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-white font-bold text-sm tracking-wide">Ganesh's AI</h3>
                                <div className="flex items-center gap-1.5">
                                    <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse shadow-[0_0_5px_#4ade80]"></span>
                                    <p className="text-blue-100 text-[10px] font-medium">Online</p>
                                </div>
                            </div>
                        </div>
                            ...
                            {messages.map((msg, index) => (
                                <div key={index} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}`}>
                                    <div className={`flex items-end gap-2 max-w-[85%]`}>
                                        ...
                                        {/* Message Bubble */}
                                        <div className={`px-4 py-3 text-sm rounded-2xl shadow-md leading-relaxed ${msg.sender === "user"
