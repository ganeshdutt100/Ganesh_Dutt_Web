...
const Testimonials = () => {
    ...
    // --- SLIDER SETTINGS (3D Effect Logic) ---
    const settings = {
        dots: true, // Dots on
        className: "center",
        centerMode: true, // Center card focus
        infinite: reviews.length > 2, // Scroll tabhi karega jab 3 se zyada item ho
        centerPadding: "0px",
        slidesToShow: 3,
        speed: 500,
        autoplay: true,
        autoplaySpeed: 3000,
        focusOnSelect: true, // Side card pe click karne se wo beech mein aayega
        arrows: false,
        responsive: [
            {
                breakpoint: 1024, // Tablet
                settings: { slidesToShow: 1, centerMode: true, centerPadding: "100px" }
            },
            {
    ...
    const styles = `
        ...
        .slick-slide {
            ...
            transform: scale(0.85); /* Chhota karo */
            opacity: 0.4; /* Dhundhla karo */
            filter: blur(2px); /* Thoda blur */
            z-index: 1;
        }

        /* ACTIVE SLIDE (CENTER CARD) */
        .slick-center {
            transform: scale(1.1); /* Bada karo */
            opacity: 1; /* Saaf dikhao */
            filter: blur(0px); /* No blur */
            z-index: 10; /* Sabse upar */
            position: relative;
        }

        /* Center Card Styling (Glow Effect) */
        .slick-center .testimonial-card {
            background: rgba(17, 24, 39, 0.95); /* Dark background */
            border: 1px solid #3b82f6; /* Blue Border */
            box-shadow: 0px 10px 40px -10px rgba(59, 130, 246, 0.6); /* Blue Glow */
        }
    ...
    return (
        <section className="py-24 bg-gray-900 relative overflow-hidden" id="testimonials">
            ...
            <div className="max-w-7xl mx-auto px-4">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6 px-4" data-aos="fade-up">
                    <div>
                        ...
                        <p className="text-gray-400 max-w-lg text-lg">
                            Feedback from students who built their careers.
                        </p>
                    </div>
                ...
                {/* --- SLIDER --- */}
                <div className="testimonial-wrapper" data-aos="fade-up" data-aos-delay="200">
                    <Slider {...settings}>
                        {reviews.map((review) => (
                            <div key={review._id} className="py-10 px-2 outline-none">
                                <div className="testimonial-card bg-gray-800/50 backdrop-blur-md p-8 rounded-3xl border border-gray-700/50 h-[420px] flex flex-col justify-between transition-all duration-300 relative overflow-hidden cursor-grab">
                                    ...
                                    <div className="flex-1 overflow-hidden">
                                        ...
                                        <p className="text-gray-300 text-lg italic leading-relaxed line-clamp-5">
                                            "{review.message}"
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4 border-t border-gray-700/50 pt-4 mt-4">
                                        <div className="w-14 h-14 rounded-full p-[2px] bg-gradient-to-tr from-blue-500 to-purple-600 shrink-0 shadow-lg">
                                            <div className="w-full h-full rounded-full bg-gray-900 overflow-hidden flex items-center justify-center">
                                                {review.image ? (
                                                    <img src={`http://localhost:5000/uploads/${review.image}`} alt={review.name} className="w-full h-full object-cover" />
                                                ) : (
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-fadeIn">
                    <div className="bg-gray-900 w-full max-w-lg p-8 rounded-3xl border border-gray-800 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar">
                        <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white bg-gray-800 p-2 rounded-full"><FaTimes /></button>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="flex justify-center mb-4">
                                <div className="relative w-20 h-20 rounded-full bg-gray-800 border-2 border-dashed border-gray-600 flex items-center justify-center overflow-hidden cursor-pointer hover:border-blue-500">
                            ...
                            <textarea name="message" value={newReview.message} onChange={handleInputChange} placeholder="Your Review..." className="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-white focus:border-blue-500 outline-none" rows="3" required />
